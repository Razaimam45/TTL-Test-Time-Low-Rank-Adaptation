...
class TextEncoder(nn.Module):
    ...
    def forward(self, prompts, tokenized_prompts):
        ...
        # Change order from (batch, num_tokens, embed) to (num_tokens, batch_size, embed)
        x = x.permute(1, 0, 2)  # NLD -> LND
        # Perform the transformer forward pass
        x = self.transformer.encoder(x)  # Convert x to Long before applying embeddings
...
class PromptLearner(nn.Module):
    ...
    def __init__(self, clip_model, classnames, batch_size=None, 
        ...
        ctx_dim = clip_model.text_model.final_layer_norm.weight.shape.numel()
...
class ClipTestTimeTuning(nn.Module):
    def __init__(self, device, classnames, batch_size, criterion='cosine', 
        ...
        # prepare prompt.
        self.prompt_learner = PromptLearner(clip_model, classnames, batch_size, n_ctx, ctx_init, ctx_position, learned_cls)
    ...
    def forward(self, input, label=None):
        if isinstance(input, Tuple):
            ...
            return self.inference(input, label)
